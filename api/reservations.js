  ...
  try {
    if (req.method === 'GET') {
      ...
      const result = await pool.query(
        `SELECT r.*, c.name as car_name FROM reservations r LEFT JOIN cars c ON r.car_id = c.id ORDER BY r.start_date DESC`
      );
    ...
    if (req.method === 'POST') {
      ...
      const result = await pool.query(
        'INSERT INTO reservations (car_id,client_name,client_phone,start_date,end_date,status) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
    ...
    if (req.method === 'PUT') {
      ...
      await pool.query(
        'UPDATE reservations SET car_id=$1,client_name=$2,client_phone=$3,start_date=$4,end_date=$5,status=$6 WHERE id=$7',
    ...
    if (req.method === 'DELETE') {
      if (!auth(req)) return res.status(401).json({ error: 'Non autorisé' });
      await pool.query('DELETE FROM reservations WHERE id=$1', [req.query.id]);
      return res.json({ success: true });
    }
  } catch (err) {
