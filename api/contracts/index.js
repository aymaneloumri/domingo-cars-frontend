  ...
  try {
    if (req.method === 'GET' && req.query.id) {
      const result = await pool.query(
        'SELECT * FROM contracts WHERE id=$1', [req.query.id]
      );
      return res.json(result.rows[0] || null);
    }
    if (req.method === 'GET') {
      const result = await pool.query(
        `SELECT ct.*, c.name as car_name FROM contracts ct
    ...
    if (req.method === 'POST') {
      ...
      const result = await pool.query(
        ...
        [
          contract_number, d.contract_date, d.car_id, d.matricule,
          d.category, d.brand, d.model, d.client_name, d.client_dob,
          d.client_phone, d.client_cin, d.client_cin_expiry, d.client_address,
          d.client_permis, d.client_permis_expiry,
          ...
          d.depart_datetime, d.depart_km,
          d.depart_inspection || 'Aucun point signalé',
          d.depart_fuel || '1/8', d.retour_prevu || '',
          d.retour_effectif || '', d.retour_km || 0, d.retour_fuel || '',
        ]
      );
